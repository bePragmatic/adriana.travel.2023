...
app.controller('rooms_admin', ['$scope', '$http', '$rootScope', '$compile', '$filter', function($scope, $http,$rootScope, $compile, $filter) {
  ...
  $scope.full_calendar = function() {
      ...
      $('#calendar').fullCalendar({
          ...
          eventRender: function(event, element, view) {
			  ...
			  if(event.Price_pr_day){
				  value = event.Price_pr_day;
			  }
			  else{
				 value = event.title;
               }
			  console.log(event.currency);
			  $('<div class="fc-bgevent-data" data-price="'+value+'" data-notes="'+event.notes+'" data-status="'+event.description+'"> <span class="price">'+ value +'</span> <span class="notes">'+ event.notes +'</span> </div>').appendTo(element);
          }
      });
  };
  ...
  $scope.steps = ['1', '2', '3', '4', '5', '6', '7', '8','9','12', '13', '10', '11', '12'];
  $scope.add_steps = ['2', '3', '4', '5', '6', '7', '8', '9', '12', '13', '10', '11', '12'];
  $scope.step_name = ""; 
  $scope.step = 0;

...
Route::group(['middleware' => ['locale', 'session_check']], function () {
    ...
    Route::get('/explore-dalmatian-coast', 'PagesController@explore')->name('explore');
    // Route::get('/accommodation', 'PagesController@single')->name('accommodation.search');
    //  Route::get('/accommodation-single', 'PagesController@single')->name('accommodation-single');
    Route::get('/service-and-activities', 'PagesController@service_and_activities')->name('service-and-activities');
    Route::get('/transfers', 'PagesController@transfers')->name('transfers');
    Route::get('/rogoznica', 'PagesController@rogoznica')->name('rogoznica');
    Route::get('/blog', 'PagesController@blog')->name('blog');
    Route::get('/blog/{slug}', 'PagesController@post_single')->name('blog.single');
    ...
    Route::get('/checkout-success', 'PagesController@checkout_success')->name('checkout_success');
    Route::get('/rent-a-boat-one', 'PagesController@rentaboatOne')->name('rent-a-boat-one');
    Route::get('/rent-a-boat-two', 'PagesController@rentaboatTwo')->name('rent-a-boat-two');
    Route::post('/rent-a-boat-form', 'PagesController@rentboatform')->name('rent-a-boat-form');
    Route::post('/rent-a-boat-form-submit', 'PagesController@rentboatformsubmit')->name('rent-a-boat-form-submit');
    Route::get('/disabledate', 'PagesController@disabledate')->name('disable-date');
    
//Ratings
    Route::match(['get', 'post'], 'ratings', 'HomeController@ratings')->name('ratings');


    // WSPAY PAYMENT
    Route::get('payment/{id}', 'PaymentController@payment')->name('payments.payment');
    Route::post('payments/signature', 'PaymentController@getSignature')->name('payments.signature');
    Route::post('payments/currency-convert', 'PaymentController@currency_convert')->name('payments.currency.convert');

    Route::get('payments/{reservation}/{transaction}/success', 'PaymentController@success')->name('payments.success');
    Route::get('payments/{reservation}/{transaction}/cancel', 'PaymentController@cancel')->name('payments.cancel');
    Route::get('payments/{reservation}/{transaction}/error', 'PaymentController@error')->name('payments.error');
    Route::get('booking/requested/{id}', 'PaymentController@requested')->name('payments.requested');


    /**
     * Access only with authentication
     */
    // TODO: pripazit duple rute izvan middlewra
    ...
    Route::get('boatsuccess/{id}', 'PaymentController@requestedboat')->name('payments.requestedboat');
    Route::get('/emailtest', 'PaymentController@emailtesting');
    Route::match(['get', 'post'], 'payments/boat/{id?}', 'PaymentController@indexboat')->where('id', '[0-9]+')->name('payments.book');
    Route::match(['get', 'post'], 'payments/boatpayment', 'PaymentController@rentboatformsubmita');


//    Route::get('tajna', function () {
//
//
//        return view('rogoznica.payment_success');
//        //        $data = [
//        //            'author' => 'Dedede',
//        //            'en' => ['title' => 'My first post'],
//        //            'de' => ['title' => 'Mon premier post'],
//        //        ];
//        //        $post = \App\Post::create($data);
//        //
//        //        return $post;
//        return  route('payments.payment', 53);
//        return request()->all();
...
Route::group(['middleware' => ['locale', 'guest:user', 'session_check', 'protection']], function () {
    ...
    //	Route::view('login', 'rogoznica.register')->name('user_login');
...
Route::group(['middleware' => ['locale', 'session_check']], function () {
    ...
    Route::get('accommodation', 'SearchController@index')->name('search_page');
...
Route::group(['middleware' => ['locale', 'auth:user', 'session_check', 'protection']], function () {
    ...
    Route::get('z/q/{id}', 'InboxController@guest_conversation')->where('id', '[0-9]+')->name('guest.conversation');
    ...
    // Trips
    Route::get('trips/current', 'TripsController@current')->name('trips.current');
...
Route::group(['middleware' => ['locale', 'session_check', 'protection']], function () {
    // Rooms details
    Route::get('rooms/{id}', 'RoomsController@rooms_detail')->where('id', '[0-9]+')->name('accommodation.single');
    ...
    Route::post('boat', 'boatController@display_boat');
    Route::post('boat', 'boatController@display_boat');
    Route::get('rent-a-boat/{id}', 'BoatingController@rooms_detail')->where('id', '[0-9]+')->name('accommodation.single');
});

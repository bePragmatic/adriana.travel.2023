...
class Rooms extends Model
{
	...
	protected $fillable = ['summary', 'name', 'sub_name'];
	...
	protected function getRatingResult($type)
	{
        ...
        if ($reviews->count() > 0) {
            ...
            $rating_value = $reviews->sum($type) / $reviews->count();
	...
	public function room_day_price()
	{
		return $this->hasMany('App\Models\RoomDayPrice', 'room_id', 'id');
	}
...
if($result->first()){
$monthly_prices = json_decode( @$result->first()->original_night, true);
		$room_price = $monthly_prices[$current_month];
		return $room_price;
}else{

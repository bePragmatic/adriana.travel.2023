...
class PaymentHelper
{
  ...
  public function price_calculation($room_id, $checkin, $checkout, $guest_count, $special_offer_id = '', $change_reservation = '', $reservation_id = '')
  {
	...
	$rooms_per_price                    = $rooms_details->room_day_price;
	...
	$price_pr_day                         = RoomDayPrice::where('from_date','>=',$date1)
	                                      ->where('to_date','<=',$date2)
					                      ->where('room_id',$room_id)->pluck('price');
    ...
    $monthly_prices = json_decode($monthly_prices->original_night);
    $result['status']                   = 'Available';
    $result['error']                    = '';
    $result['total_nights']             = $total_nights;
    $result['per_night']                = $monthly_prices->$current_month;// $rooms_price->night;
    $result['rooms_price']              = $amount;// $rooms_price->night;
    $result['total_night_price']        = 0;
    $result['base_rooms_price']         = $monthly_prices->$current_month;////$rooms_price->night;
    $result['night_price']              = 0;
    $result['additional_guests']         = 0;
...
foreach ($days as $date) {
      ...
      if ($status && !in_array($date, $pre_reserved_days)) {
        ...
        $price = $monthly_prices->$current_month;
	...
	if($amount){
        $result['total_night_price'] += $amount;}
		else{$result['total_night_price'] += $price;}
  ...
  /**
   * Generates WsPay Signature
   *
   * @return \Illuminate\Http\JsonResponse
   */
  public function signature()
  {
    $signature = md5(config('wspay.shop_id') .
      config('wspay.secret_key') .
      request()->order_id .
      config('wspay.secret_key') .
      request()->price .
      config('wspay.secret_key'));
   ...
   * @return bool
   */
  public function verifyWsPaySignature()
  {
    $signature = md5(config('wspay.shop_id') .
      config('wspay.secret_key') .

...
class InboxController extends Controller
{
    ...
    public function guest_conversation(Request $request)
    {
        ...
        $reservation = Reservation::with('users')->where('id', $request->id)->userRelated()->first();
        if(!$reservation)
            abort(404);
    ...
    public function reply(Request $request, EmailController $email_controller)
    {
        ...
        if($reservation_details->user_id == Auth::user()->id) {
            ...
            echo '<div class="row my-4"> <div class="col-10"> <div class="card bg-color-msg"> <div class="card-body bg-color-msg custom-arrow-guest right"> <span class="message-text">'.$message.'</span> </div> </div> <div class="my-2 time text-right"> <span> '.$messages->created_time.' </span> </div> </div> <div class="col-2 pl-0"> <img src="'.Auth::user()->profile_picture->src.'" class="user-profile-photo round"> </div> </div>';
        }
        else if($reservation_details->rooms->user_id == Auth::user()->id)
        {
            ...
            echo $html.'<div class="row my-4"> <div class="col-3 col-md-2 pr-0 text-center"> <a aria-label="'.Auth::user()->first_name.'" data-behavior="tooltip" href="'.url('/').'/users/show/'.Auth::user()->id.'"> <img title="'.Auth::user()->first_name.'" src="'.Auth::user()->profile_picture->src.'" alt="'.Auth::user()->first_name.'"> </a> </div> <div class="col-9 col-md-10"> <div class="card bg-color-msg custom-arrow-guest left"> <div class="card-body p-3"> <p>'.$message.'</p> </div> </div> <div class="time-container"> <small title="'.$messages->created_at.'" class="time">'. $messages->created_time .'</small> <small class="exact-time d-none">"'.$messages->created_at.'"</small> </div> </div> </div> </li>';
        }
    }

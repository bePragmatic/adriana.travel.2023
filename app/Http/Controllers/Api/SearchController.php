...
class SearchController extends Controller
{
	...
	function explore_details(Request $request)
	{
		...
		if ($request->page != '' && $request->page != '0') {
			...
			if (isset($request->latitude) && isset($request->longitude)) {
				$rooms = $rooms->whereHas('rooms_address', function ($query) use ($request) {
			...
			if ($totalcount->total == 0 || empty($totalcount->data)) {
				...
				for ($i = 0; $i < $count; $i++) {
					...
					@$result_value[] = array(
						...
						'rating' => $data_result['data'][$i]['overall_star_rating']['rating_value'] != null

							? (string) $data_result['data'][$i]['overall_star_rating']['rating_value']

							: '0',

						'reviews_count' => $data_result['data'][$i]['reviews_count'] != null

							? $data_result['data'][$i]['reviews_count'] : 0,

						'instant_book' => $data_result['data'][$i]['booking_type'] == 'instant_book'

							? 'Yes' : 'No',

						'latitude' => (string) $data_result['data'][$i]['rooms_address']['latitude'],

						'longitude' => (string) $data_result['data'][$i]['rooms_address']['longitude'],

						'is_wishlist' => $data_result['data'][$i]['overall_star_rating']['is_wishlist'] != null

							? $data_result['data'][$i]['overall_star_rating']['is_wishlist'] : 'No',

						'country_name' => $data_result['data'][$i]['rooms_address']['country_name'],
	...
	public function homePage()
	{
		...
		if (!request()->search_type || request()->search_type == "stay" || request()->search_type == "all") {
			$explore_[] = ['name' => trans('messages.home.stays'), 'key' => 'Homes', 'image' => view()->shared('home_page_stay_image')];
		}

...
use App\Models\UsersVerificatifonDocuments;
...
class UserController extends Controller
{
    ...
    public function update_payout_preferences(Request $request, EmailController $email_controller, $id)
    {
        ...
        if (strpos($url, "localhost") > 0) {
            $url = 'http://support@tempusmedia.hr';
        }
    ...
    public function transaction_history_csv(Request $request)
    {
        ...
        for ($i = 0; $i < count($transaction); $i++) {
            unset($transaction[$i]['id']);
            unset($transaction[$i]['reservation_id']);
            unset($transaction[$i]['room_id']);
            unset($transaction[$i]['user_id']);
            unset($transaction[$i]['status']);
            unset($transaction[$i]['penalty_id']);
            unset($transaction[$i]['penalty_amount']);
            unset($transaction[$i]['created_at']);
            unset($transaction[$i]['updated_at']);
            unset($transaction[$i]['correlation_id']);
            unset($transaction[$i]['currency_symbol']);

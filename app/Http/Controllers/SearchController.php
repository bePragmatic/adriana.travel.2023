...
class SearchController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if($current_refinement=="Homes")
        {
           ...
           // return $data;
            return view('rogoznica.search', compact('data'));
        }
        else
        {
          ...
          //  return view('host_experiences.search', $data);
            ...
            return view('rogoznica.search', $data);
        }
    }
    ...
    public function searchResult(Request $request) {
       ...
       $rooms = $rooms->orderByRaw('RAND(1234)')->paginate($per_page)->toJson();
    
      $rooms = json_decode($rooms);
   
      $current_month = strtolower(date('F'));
    foreach($rooms->data as $single){
       
     
        $monthly_prices = json_decode( $single->rooms_price->original_night, true);
        $single->rooms_price->original_night = $monthly_prices[$current_month];
        $single->rooms_price->night = $monthly_prices[$current_month];
    }
